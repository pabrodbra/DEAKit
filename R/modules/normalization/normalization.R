...
normalization.core <- function(){
  ...
  metadata$hk_nf <- hk.factor(normalization.values$ncounts, lod) # Housekeeping normalization
  normalization.values$ncounts <- normalization.values$ncounts %*% diag(metadata$hk_nf) # Housekeeping normalization
  colnames(normalization.values$ncounts) <- colnames(counts)
