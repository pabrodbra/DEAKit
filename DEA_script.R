...
if(!is.null(opt$output) && !file.exists(opt$output)) {
  stop("An existent output directory must be specified")
}

if(!is.null(opt$images) && !file.exists(opt$images)) {
  stop("An existent output images directory must be specified")
}

if(!is.null(opt$docs) && !file.exists(opt$docs)) {
  stop("An existent output docs directory must be specified")
}
...
if(is.null(opt$output)) {
  DIR.OUTPUT <- file.path(getwd(), "output")
  dir.create(DIR.OUTPUT, showWarnings = FALSE)
} else DIR.OUTPUT <- opt$output

if(is.null(opt$images)) {
  DIR.IMAGES <- file.path(DIR.OUTPUT, "images")
  dir.create(DIR.IMAGES, showWarnings = FALSE)
} else DIR.IMAGES <- opt$images

if(is.null(opt$docs)) {
  DIR.DOCS <- file.path(DIR.OUTPUT, "docs")
  dir.create(DIR.DOCS, showWarnings = FALSE)
} else DIR.DOCS <- opt$docs
...
BASE.OUTPUT.NAME <- file.path(DIR.OUTPUT, opt$label)
...
pdf(file.path(DIR.OUTPUT, "FOV.pdf"))
plot(fov.plot)
invisible(dev.off())
...
bd.plot <- plotBD(eset = eset, metadata = metadata, y.thresholds = c(opt$bd_min, opt$bd_max),
                  comparison.key = key.label, legend.label = opt$label)
pdf(file.path(DIR.OUTPUT, "BD.pdf"))
plot(bd.plot)
invisible(dev.off())

# Positive Controls
pdf(file.path(DIR.OUTPUT, "Positive-Control.pdf"))
plot(boxplot.expr(eset, is.positive))
invisible(dev.off())

# Negative Controls
pdf(file.path(DIR.OUTPUT, "Negative-Control.pdf"))
...
pdf(file.path(DIR.OUTPUT, "Housekeeping-Genes.pdf"))
...
pdf(file.path(DIR.OUTPUT, "Housekeeping-Samples.pdf"))
...
pdf(file.path(DIR.OUTPUT, "Housekeeping-Endogenous.pdf"))
...
pdf(file.path(DIR.OUTPUT, "Pre-Normalization.pdf"))
...
metadata$hk_nf <- hk.factor(counts, lod)
...
pdf(file.path(DIR.OUTPUT, "Housekeeping-Normalization.pdf"))
...
pdf(file.path(DIR.OUTPUT, "Samples-PCA.pdf"))
...
pdf(file.path(DIR.OUTPUT, "Dotplot-All.pdf"))
...
pdf(file.path(DIR.OUTPUT, "Dotplot-Over.pdf"))
...
pdf(file.path(DIR.OUTPUT, "Dotplot-Under.pdf"))
...
MARKDOWN.PATH <- opt$report
MARKDOWN.OUTPUT <- file.path(DIR.DOCS, "DEA_report.html")
...
rmarkdown::render(MARKDOWN.PATH, output_file = MARKDOWN.OUTPUT)
